...
# -----------------------------------------------------------------------------
# Every string here must be present in the feature table built by Pipe-1.
# The training notebook asserts this before collecting any pandas frame.
...
FEATURE_BLOCKS = {
    "lags": FEATURES_LAGS,
    "rolling": FEATURES_ROLLING,
    "lag_bands": FEATURES_LAG_BANDS,
    "pair": FEATURES_PAIR,
    "seasonal": FEATURES_SEASONAL,
    "agency_article": FEATURES_AGENCY_ARTICLE,
    "calendar": FEATURES_CALENDAR,
    "static": FEATURES_STATIC_NUMERIC,
    "billing": FEATURES_BILLING,
    "categorical": FEATURES_CATEGORICAL,
}

FEATURES_BATCH_UNSAFE = (
    [f"lag_{n}" for n in LAGS_ALL if n < 26]
    + FEATURES_ROLLING
    + [
        "pair_mean", "pair_median", "pair_max", "pair_min", "pair_count",
        "pair_std", "pair_cv", "pair_zero_rate_expanding",
...
FEATURES_BATCH_SAFE = [
    f for f in FEATURES
    if f not in set(FEATURES_BATCH_UNSAFE)
]
...
FEATURE_SELECTION_MAX_TRAIN_ROWS = 1_200_000
PERMUTATION_MAX_VAL_ROWS = 180_000
PERMUTATION_MAX_FEATURES = 120
FEATURE_SELECTION_STEPS = [180, 140, 110, 90, 75]
FEATURE_SELECTION_NUM_ROUNDS_ZERO = 900
FEATURE_SELECTION_NUM_ROUNDS_QTY = 1200
FEATURE_SELECTION_EARLY_STOP_ZERO = 40
FEATURE_SELECTION_EARLY_STOP_QTY = 50
